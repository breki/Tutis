...
namespace CleanCode.Step3
{
    public class Parser
    {
        ...
        {
            string htmlData = FetchHtml(isWeb, source);

            int cursor = htmlData.IndexOf(TableMarker);
            cursor = htmlData.IndexOf(TBodyMarker, cursor);

            return ExtractWeatherFromHtml(htmlData, cursor);
        }

        private string FetchHtml(bool isWeb, string source)
        {
            string htmlData;
        ...
        private WeatherData ExtractWeatherFromHtml(string htmlData, int cursor)
        {
