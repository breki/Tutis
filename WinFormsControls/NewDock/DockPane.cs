...
namespace NewDock
{
    ...
    public class DockPane : UserControl
    {
        ...
        public IDisposable AutoHidePane
        {
        ...
        public bool HasTabPageContextMenu
        {
            get { return TabPageContextMenu != null; }
        }

        public void ShowTabPageContextMenu(Control control, Point position)
        {
        ...
        public void SetIsActiveDocumentPane(bool value)
        {
        ...
        public DockPaneAppearanceStyle Appearance
        {
            get { return (DockState == DockState.Document) ? DockPaneAppearanceStyle.Document : DockPaneAppearanceStyle.ToolWindow; }
        }
        ...
        private DockPaneHitTestResult GetHitTest(Point ptMouse)
        {
            Point ptMouseClient = PointToClient(ptMouse);

            Rectangle rectCaption = CaptionRectangle;
            if (rectCaption.Contains(ptMouseClient))
                return new DockPaneHitTestResult(DockPaneHitTestArea.Caption, -1);

            Rectangle rectContent = ContentRectangle;
            if (rectContent.Contains(ptMouseClient))
                return new DockPaneHitTestResult(DockPaneHitTestArea.Content, -1);

            Rectangle rectTabStrip = TabStripRectangle;
            if (rectTabStrip.Contains(ptMouseClient))
                return new DockPaneHitTestResult(DockPaneHitTestArea.TabStrip, TabStripControl.HitTest(TabStripControl.PointToClient(ptMouse)));

            return new DockPaneHitTestResult(DockPaneHitTestArea.None, -1);
        }
        ...
        public void ValidateActiveContent()
        {
        ...
        public DockPane DockTo(INestedPanesContainer container, DockPane previousPane, DockAlignment alignment, double proportion)
        {
            ...
            DockPane pane;
            ((DockContent)DockPanel.DummyContent).DockPanel = DockPanel;
            ...
            ((DockContent)DockPanel.DummyContent).DockPanel = null;
