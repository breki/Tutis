...
namespace App
{
    public class RightAnchorConstraint : IRelationship
    {
        public RightAnchorConstraint(ILayoutElement source, ILayoutElement target)
        {
            sources.Add(source);
            targets.Add(target);
        }

        public RightAnchorConstraint(IEnumerable<ILayoutElement> sources, IEnumerable<ILayoutElement> targets)
        {
        ...
        public void Apply()
        {
            ...
            foreach (ILayoutElement target in Targets)
            {
                try
                {
                    target.X = anchorPosition - target.Width;
                }
        ...
        public void Check()
        {
            ...
            foreach (ILayoutElement target in Targets)
            {
                ...
                if (elementRight > anchorPosition)
                    throw new ConstraintViolationException(
                        "Element {0} should be right-anchored to {1} but it is right position is at {2} instead",
                        target.Name,
                        anchorPosition,
                        elementRight);
            }
